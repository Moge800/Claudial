...
> **トークン消費について**
> デーモンはポーリングのたびに `claude-haiku-4-5-20251001` へ最小限のAPIコール（input 8 + output 1 トークン）を行い、レスポンスのレートリミットヘッダーから使用率を取得します。デフォルトの 60 秒間隔では約 $0.000015/日 の消費で、Pro プランの日割り料金の 0.002% 未満です。
